...
// 根据id查询用户信息
export const getUser = (userId:string) => request.get(`/sysUser/getUser?userId=${userId}`)

// 删除用户信息
export const deleteUser = (data:any) => request.post('/sysUser/deleteUser', data)

// 批量删除用户信息
export const deleteUsers = (userIds:string) => request.get(`/sysUser/deleteUsers?userIds=${userIds}`)
...
/**-------------------------------角色------------------------------- */
// 删除角色信息
export const deleteRole = (roleId:any) => request.get(`/sysRole/deleteRole?roleId=${roleId}`)
...
// 删除菜单
export const deleteMenu = (menuId:number) => request.get(`/menu/delete/${menuId}`)

// 查询菜单详情
export const getMenu = (menuId:number) => request.get(`/menu/detail/${menuId}`)
...
// 查询全部菜单权限
export const getMenulist = () => request.post('/menu/getMenulist')
...
// 根据token查询用户菜单
export const getMenuByToken = () => request.get('/menu/query')

// 批量删除菜单信息 {menuIds:}
export const deleteMenus = (menuIds:any) => request.post(`/menu/deleteMenus?menuIds=${menuIds}`)
...
*/
export const getDictionaryDetail = (id:number) => request.get(`/dictionary/detail/${id}`)

// 按条件查询字典项
export const getAllDictionaryList = (data:any) => request.post('/dictionary/list', data)

// 按条件查询字典项
export const getDictionaryOption = async(parentCode:string) => {
 const {result, code}:any =  await request.post('/dictionary/list', { "parentCode": parentCode})
 if(code===200){
...
// 删除字典项
export const deleteDictionary = (id:number) => request.get(`/dictionary/delete/${id}`)
...
// 批量删除字典信息 {dictIds:}
export const deleteDicts = (dictIds:string) => request.post(`/dictionary/deleteDicts?dictIds=${dictIds}`)
...
// 获取单位树
export const getOrganizationTtree = () => request.get('/sysOrganization/treeAll')
