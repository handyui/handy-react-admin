...
const PreferentialPage: FC = (props) => {
    ...
    const handleDetail=async(record:any)=>{
       // 先校验，存在就删除，再打开新的编辑页
       checkUrl('/soldierManage/senior/detail/:id')
       history.push({
            pathname:`/soldierManage/senior/detail/${record.preferentialId}`,
            state:{
                refresh: true   
            }
       })
            ...
            },
            {
                ...
                actions: [
                    ...
                    },
                    {
                        ...
                        func: async ({record}:any, callback:any) => {
                            // 先校验，存在就删除，再打开新的编辑页
                            checkUrl('/soldierManage/senior/edit/:id')
                            history.push({
                                pathname:`/soldierManage/senior/edit/${record.preferentialId}`,
                                state:{
                                    refresh: true   
                                }
                            })
                    ...
                    },
                    {
                        ...
                        func: async ({record}:any, callback:any) => {
                            // 先校验，存在就删除，再打开新的编辑页
                            checkUrl('/soldierManage/senior/audit/:id')
                            history.push({
                                pathname:`/soldierManage/senior/audit/${record.preferentialId}`,
                                state:{
                                    refresh: true   
                                }
                            })
    ...
    const addItem =()=>{
        history.push({
            pathname:'/soldierManage/senior/add'
        })
    }
    ...
    useEffect(()=>{
        if(state && (state as any).refresh && pathname === '/soldierManage/senior'){
