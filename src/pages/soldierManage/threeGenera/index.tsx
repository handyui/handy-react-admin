...
const PreferentialPage: FC = (props) => {
    ...
    const handleDetail=async(record:any)=>{
       // 先校验，存在就删除，再打开新的编辑页
       checkUrl('/soldierManage/threeGenera/detail/:id')
       history.push({
            pathname:`/soldierManage/threeGenera/detail/${record.preferentialId}`,
            state:{
                refresh: true   
            }
       })
            ...
            },
            {
                ...
                actions: [
                    ...
                    },
                    {
                        ...
                        func: async ({record}:any, callback:any) => {
                            // 先校验，存在就删除，再打开新的编辑页
                            checkUrl('/soldierManage/threeGenera/edit/:id')
                            history.push({
                                pathname:`/soldierManage/threeGenera/edit/${record.preferentialId}`,
                                state:{
                                    refresh: true   
                                }
                            })
                    ...
                    },
                    {
                        ...
                        func: async ({record}:any, callback:any) => {
                            // 先校验，存在就删除，再打开新的编辑页
                            checkUrl('/soldierManage/threeGenera/audit/:id')
                            history.push({
                                pathname:`/soldierManage/threeGenera/audit/${record.preferentialId}`,
                                state:{
                                    refresh: true   
                                }
                            })
    ...
    const addItem =()=>{
        history.push({
            pathname:'/soldierManage/threeGenera/add'
        })
    }
    ...
    useEffect(()=>{
        if(state && (state as any).refresh && pathname === '/soldierManage/threeGenera'){
