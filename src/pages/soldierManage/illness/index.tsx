...
const PreferentialPage: FC = (props) => {
    ...
    const handleDetail=async(record:any)=>{
       // 先校验，存在就删除，再打开新的编辑页
       checkUrl('/soldierManage/illness/detail/:id')
       history.push({
            pathname:`/soldierManage/illness/detail/${record.preferentialId}`,
            state:{
                refresh: true   
            }
       })
            ...
            },
            {
                ...
                actions: [
                    ...
                    },
                    {
                        ...
                        func: async ({record}:any, callback:any) => {
                            // 先校验，存在就删除，再打开新的编辑页
                            checkUrl('/soldierManage/illness/edit/:id')
                            history.push({
                                pathname:`/soldierManage/illness/edit/${record.preferentialId}`,
                                state:{
                                    refresh: true   
                                }
                            })
                    ...
                    },
                    {
                        ...
                        func: async ({record}:any, callback:any) => {
                            // 先校验，存在就删除，再打开新的编辑页
                            checkUrl('/soldierManage/illness/audit/:id')
                            history.push({
                                pathname:`/soldierManage/illness/audit/${record.preferentialId}`,
                                state:{
                                    refresh: true   
                                }
                            })
    ...
    const addItem =()=>{
        history.push({
            pathname:'/soldierManage/illness/add'
        })
    }
    ...
    useEffect(()=>{
        if(state && (state as any).refresh && pathname === '/soldierManage/illness'){
