...
const PreferentialPage: FC = (props) => {
    ...
    const handleDetail=async(record:any)=>{
       // 先校验，存在就删除，再打开新的编辑页
       checkUrl('/soldierManage/preferential/detail/:id')
       history.push({
            pathname:`/soldierManage/preferential/detail/${record.preferentialId}`,
            state:{
                refresh: true   
            }
       })
            ...
            },
            {
                ...
                actions: [
                    ...
                    },
                    {
                        ...
                        func: async ({record}:any, callback:any) => {
                            // 先校验，存在就删除，再打开新的编辑页
                            checkUrl('/soldierManage/preferential/edit/:id')
                            history.push({
                                pathname:`/soldierManage/preferential/edit/${record.preferentialId}`,
                                state:{
                                    refresh: true   
                                }
                            })
                    ...
                    },
                    {
                        ...
                        func: async ({record}:any, callback:any) => {
                            // 先校验，存在就删除，再打开新的编辑页
                            checkUrl('/soldierManage/preferential/audit/:id')
                            history.push({
                                pathname:`/soldierManage/preferential/audit/${record.preferentialId}`,
                                state:{
                                    refresh: true   
                                }
                            })
    ...
    const addItem =()=>{
        history.push({
            pathname:'/soldierManage/preferential/add'
        })
    }

    // const handleClick=(val:object)=>{
    //     (tableRef as any).current.refreshTable(val)
    // }

    // useEffect(()=>{
    //     if(state && (state as any).refresh){
    //         (searchFormRef as any ).current.refreshData()
    //     }
    // }, [state])
    ...
    useEffect(()=>{
        if(state && (state as any).refresh && pathname === '/soldierManage/preferential'){
    ...
    return (
        <div className="content">
            <CustSchemaForm ref={searchFormRef} dynamicValidateForm={searchSchema} callback={handleClick} showBtn>
             {/* <DatePicker defaultValue={moment('2015-01-01', 'YYYY-MM-DD')} format="YYYY-MM-DD HH:mm" onChange={handleChange}/> */}
