...
const PreferentialPage: FC = (props) => {
    ...
    const handleDetail=async(record:any)=>{
       // 先校验，存在就删除，再打开新的编辑页
       checkUrl('/soldierManage/joinBattle/detail/:id')
       history.push({
            pathname:`/soldierManage/joinBattle/detail/${record.preferentialId}`,
            state:{
                refresh: true   
            }
       })
            ...
            },
            {
                ...
                actions: [
                    ...
                    },
                    {
                        ...
                        func: async ({record}:any, callback:any) => {
                            // 先校验，存在就删除，再打开新的编辑页
                            checkUrl('/soldierManage/joinBattle/edit/:id')
                            history.push({
                                pathname:`/soldierManage/joinBattle/edit/${record.preferentialId}`,
                                state:{
                                    refresh: true   
                                }
                            })
                    ...
                    },
                    {
                        ...
                        func: async ({record}:any, callback:any) => {
                            // 先校验，存在就删除，再打开新的编辑页
                            checkUrl('/soldierManage/joinBattle/audit/:id')
                            history.push({
                                pathname:`/soldierManage/joinBattle/audit/${record.preferentialId}`,
                                state:{
                                    refresh: true   
                                }
                            })
    ...
    const addItem =()=>{
        history.push({
            pathname:'/soldierManage/joinBattle/add'
        })
    }
    ...
    useEffect(()=>{
        if(state && (state as any).refresh && pathname === '/soldierManage/joinBattle'){
